...
def start_mock_server(host='127.0.0.1', port=55555):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        print("Waiting for your C client to connect tomorrow...\n")
        ...
        with conn:
            ...
            start_header = struct.pack('!BBH', MAGIC, TYPE_START, len(start_payload))
            ...
            for i in range(1, 4):
                ...
                data_header = struct.pack('!BBH', MAGIC, TYPE_DATA, len(data_payload))

                conn.sendall(data_header + data_payload)
                time.sleep(1)

            print("Sending END packet...")
            end_header = struct.pack('!BBH', MAGIC, TYPE_END, 0)
